...
<title>Students — DriveEase Admin</title>
...
<style>
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  .logo-text { font-family:'Playfair Display',serif; font-size:1.15rem; font-weight:700; color:var(--text); }
  ...
  .section-heading { font-family:'Playfair Display',serif; font-size:1.4rem; font-weight:700; }
  ...
  .btn-action.primary { background:var(--gold); color:#0d0d0d; border-color:var(--gold); font-weight:600; }
  ...
  .badge { display:inline-flex; align-items:center; gap:5px; padding:4px 11px; border-radius:999px; font-size:0.68rem; font-weight:600; letter-spacing:0.07em; text-transform:uppercase; }
  .badge-blue { background:rgba(74,158,255,0.12); color:var(--blue); }
  .badge-gold { background:rgba(212,160,23,0.12); color:var(--gold-light); }
  .badge-purple { background:rgba(160,100,255,0.12); color:#c084fc; }
  ...
  .btn-icon.edit:hover { border-color:var(--blue); color:var(--blue); background:rgba(74,158,255,0.08); }
  .btn-icon.del:hover { border-color:var(--red); color:var(--red); background:rgba(224,82,82,0.08); }
  .btn-icon svg { width:13px; height:13px; stroke:currentColor; fill:none; stroke-width:2; }
  .back-link { display:inline-flex; align-items:center; gap:6px; font-size:0.78rem; color:var(--muted); text-decoration:none; margin-top:20px; transition:color 0.2s; }
...
<aside class="sidebar">
  <div class="sidebar-logo">
    ...
    <div><div class="logo-text">Drive<span>Ease</span></div><div class="logo-sub">Admin Panel</div></div>
  </div>
  ...
  <a href="StudentServlet?action=list" class="nav-item active"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>Students</a>
  ...
  <a href="TestServlet?action=list" class="nav-item"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>Driving Tests</a>
  <div class="sidebar-footer">
    <a href="AdminServlet?action=logout" class="nav-item logout"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/></svg>Logout</a>
  </div>
</aside>
<div class="main">
  <div class="topbar">
    <span class="topbar-title">Student <span style="color:var(--gold);font-style:italic">Management</span></span>
    <div class="admin-chip"><span class="admin-dot"></span>Admin: <%= session.getAttribute("adminUser") %></div>
  </div>
  <div class="content">
    <div class="section-header">
      <h2 class="section-heading">Student <em>Management</em></h2>
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>Name</th><th>NIC</th><th>Phone</th><th>License Type</th><th>Actions</th></tr></thead>
        <tbody>
          <c:forEach var="student" items="${listStudent}">
            <tr>
              <td><span class="id-pill"><c:out value="${student.studentId}" /></span></td>
              <td class="bold"><c:out value="${student.name}" /></td>
              <td><c:out value="${student.nic}" /></td>
              <td><c:out value="${student.phoneNumber}" /></td>
              <td><span class="badge ${student.licenseType == 'Car' ? 'badge-blue' : (student.licenseType == 'Bike' ? 'badge-gold' : 'badge-purple')}"><c:out value="${student.licenseType}" /></span></td>
              <td><div class="action-btns">
                <a href="StudentServlet?action=edit&id=<c:out value='${student.studentId}' />" class="btn-icon edit"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></a>
                <a href="StudentServlet?action=delete&id=<c:out value='${student.studentId}' />" class="btn-icon del" onclick="return confirm('Delete this student?');"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></a>
              </div></td>
            </tr>
          </c:forEach>
          <c:if test="${empty listStudent}">
            <tr><td colspan="6"><div class="empty-row">No students registered yet.</div></td></tr>
          </c:if>
        </tbody>
      </table>
    </div>
    <a href="AdminServlet?action=dashboard" class="back-link">
