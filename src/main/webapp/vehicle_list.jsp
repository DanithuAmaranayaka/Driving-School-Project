...
<title>Vehicles — DriveEase Admin</title>
...
<style>
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
  ...
  .badge-green{background:rgba(62,207,142,0.12);color:var(--green)}.badge-green .badge-dot{background:var(--green);box-shadow:0 0 6px var(--green)}
  .badge-gold{background:rgba(212,160,23,0.12);color:var(--gold-light)}.badge-red{background:rgba(224,82,82,0.12);color:var(--red)}
...
<aside class="sidebar">
  ...
  <a href="VehicleServlet?action=list" class="nav-item active"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 17H3a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v9a2 2 0 01-2 2h-3"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg>Vehicles</a>
...
<div class="main">
  <div class="topbar">
    <span class="topbar-title">Vehicle <span style="color:var(--gold);font-style:italic">Fleet</span></span>
    <div class="admin-chip"><span class="admin-dot"></span>Admin: <%= session.getAttribute("adminUser") %></div>
  </div>
  <div class="content">
    <div class="section-header">
      <h2 class="section-heading">Vehicle <em>Fleet</em></h2>
      <a href="VehicleServlet?action=new" class="btn-action primary"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Add New Vehicle</a>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>Model</th><th>Plate Number</th><th>Type</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          <c:forEach var="vehicle" items="${listVehicle}">
            <tr>
              <td><span class="id-pill"><c:out value="${vehicle.vehicleId}"/></span></td>
              <td class="bold"><c:out value="${vehicle.model}"/></td>
              <td style="font-family:monospace"><c:out value="${vehicle.plateNumber}"/></td>
              <td><c:out value="${vehicle.vehicleType}"/></td>
              <td><span class="badge ${vehicle.status=='Available'?'badge-green':(vehicle.status=='In Lesson'?'badge-gold':'badge-red')}"><span class="badge-dot"></span><c:out value="${vehicle.status}"/></span></td>
              <td><div class="action-btns">
                <a href="VehicleServlet?action=edit&id=<c:out value='${vehicle.vehicleId}'/>" class="btn-icon edit"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></a>
                <a href="VehicleServlet?action=delete&id=<c:out value='${vehicle.vehicleId}'/>" class="btn-icon del" onclick="return confirm('Delete this vehicle?');"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></a>
              </div></td>
            </tr>
          </c:forEach>
          <c:if test="${empty listVehicle}"><tr><td colspan="6"><div class="empty-row">No vehicles in the fleet.</div></td></tr></c:if>
        </tbody>
      </table>
    </div>
