...
<title>Payments — DriveEase Admin</title>
...
<style>
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
  ...
  .amount-gold{color:var(--gold-light);font-weight:600;font-family:'Playfair Display',serif}
...
<aside class="sidebar">
  ...
  <a href="PaymentServlet?action=list" class="nav-item active"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2"/><path d="M1 10h22"/></svg>Payments</a>
...
<div class="main">
  <div class="topbar">
    <span class="topbar-title">Payment <span style="color:var(--gold);font-style:italic">History</span></span>
    <div class="admin-chip"><span class="admin-dot"></span>Admin: <%= session.getAttribute("adminUser") %></div>
  </div>
  <div class="content">
    <div class="section-header">
      <h2 class="section-heading">Payment <em>History</em></h2>
      <a href="PaymentServlet?action=new" class="btn-action primary"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Record New Payment</a>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>Student</th><th>Course Type</th><th>Amount</th><th>Date</th><th>Method</th><th>Actions</th></tr></thead>
        <tbody>
          <c:forEach var="payment" items="${listPayment}">
            <tr>
              ...
              <td><div class="action-btns">
                <a href="PaymentServlet?action=edit&id=<c:out value='${payment.paymentId}'/>" class="btn-icon edit"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></a>
                <a href="PaymentServlet?action=delete&id=<c:out value='${payment.paymentId}'/>" class="btn-icon del" onclick="return confirm('Delete this payment record?');"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></a>
              </div></td>
            </tr>
          </c:forEach>
          <c:if test="${empty listPayment}"><tr><td colspan="7"><div class="empty-row">No payments recorded.</div></td></tr></c:if>
        </tbody>
      </table>
    </div>
