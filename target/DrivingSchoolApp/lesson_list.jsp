...
<title>Lessons — DriveEase Admin</title>
...
<style>
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
  ...
  .badge-sched{background:rgba(212,160,23,0.12);color:var(--gold-light)}.badge-sched .badge-dot{background:var(--gold)}
  .badge-done{background:rgba(62,207,142,0.12);color:var(--green)}.badge-done .badge-dot{background:var(--green);box-shadow:0 0 6px var(--green)}
  .badge-cancel{background:rgba(224,82,82,0.12);color:var(--red)}.badge-cancel .badge-dot{background:var(--red)}
...
<aside class="sidebar">
  ...
  <a href="LessonServlet?action=list" class="nav-item active"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>Lessons</a>
...
<div class="main">
  <div class="topbar">
    <span class="topbar-title">Lesson <span style="color:var(--gold);font-style:italic">Schedule</span></span>
    <div class="admin-chip"><span class="admin-dot"></span>Admin: <%= session.getAttribute("adminUser") %></div>
  </div>
  <div class="content">
    <div class="section-header">
      <h2 class="section-heading">Lesson <em>Schedule</em></h2>
      <a href="LessonServlet?action=new" class="btn-action primary"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Schedule New Lesson</a>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>Student</th><th>Instructor</th><th>Vehicle</th><th>Date</th><th>Time</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          <c:forEach var="lesson" items="${listLesson}">
            <tr>
              ...
              <td><span class="badge ${lesson.status=='Scheduled'?'badge-sched':(lesson.status=='Completed'?'badge-done':'badge-cancel')}"><span class="badge-dot"></span><c:out value="${lesson.status}"/></span></td>
              <td><div class="action-btns">
                <a href="LessonServlet?action=edit&id=<c:out value='${lesson.lessonId}'/>" class="btn-icon edit"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></a>
                <a href="LessonServlet?action=delete&id=<c:out value='${lesson.lessonId}'/>" class="btn-icon del" onclick="return confirm('Delete this lesson?');"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></a>
              </div></td>
            </tr>
          </c:forEach>
          <c:if test="${empty listLesson}"><tr><td colspan="8"><div class="empty-row">No lessons scheduled.</div></td></tr></c:if>
        </tbody>
      </table>
    </div>
