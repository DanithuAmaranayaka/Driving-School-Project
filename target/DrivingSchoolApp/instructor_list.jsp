...
<title>Instructors — DriveEase Admin</title>
...
<style>
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
  ...
  .badge-blue{background:rgba(74,158,255,0.12);color:var(--blue)}.badge-gold{background:rgba(212,160,23,0.12);color:var(--gold-light)}.badge-purple{background:rgba(160,100,255,0.12);color:#c084fc}
...
<aside class="sidebar">
  ...
  <a href="InstructorServlet?action=list" class="nav-item active"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Instructors</a>
...
<div class="main">
  <div class="topbar">
    <span class="topbar-title">Instructor <span style="color:var(--gold);font-style:italic">Management</span></span>
    <div class="admin-chip"><span class="admin-dot"></span>Admin: <%= session.getAttribute("adminUser") %></div>
  </div>
  <div class="content">
    <div class="section-header">
      <h2 class="section-heading">Instructor <em>Management</em></h2>
      <a href="InstructorServlet?action=new" class="btn-action primary"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Add New Instructor</a>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>Name</th><th>Phone</th><th>Vehicle Type</th><th>Actions</th></tr></thead>
        <tbody>
          <c:forEach var="instructor" items="${listInstructor}">
            <tr>
              <td><span class="id-pill"><c:out value="${instructor.instructorId}"/></span></td>
              <td class="bold"><c:out value="${instructor.name}"/></td>
              <td><c:out value="${instructor.phoneNumber}"/></td>
              <td><span class="badge ${instructor.vehicleType=='Car'?'badge-blue':(instructor.vehicleType=='Bike'?'badge-gold':'badge-purple')}"><c:out value="${instructor.vehicleType}"/></span></td>
              <td><div class="action-btns">
                <a href="InstructorServlet?action=edit&id=<c:out value='${instructor.instructorId}'/>" class="btn-icon edit"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></a>
                <a href="InstructorServlet?action=delete&id=<c:out value='${instructor.instructorId}'/>" class="btn-icon del" onclick="return confirm('Delete instructor?');"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></a>
              </div></td>
            </tr>
          </c:forEach>
          <c:if test="${empty listInstructor}"><tr><td colspan="5"><div class="empty-row">No instructors added yet.</div></td></tr></c:if>
        </tbody>
      </table>
    </div>
